    ...
    internal class Program
    {
        private static int Main(string[] args)
        {
            ...
            try
            {
                ...
                if (files.Count == 1 && Path.GetExtension(files[0]) == ".mpxproj")
                {
                ...
                else
                {
                    ...
                    {
                        Console.WriteLine("ModPhuserEx.CLI: No output directory specified.");
                        PrintUsage();
                        return -1;
                    }
                    ...
                    if (Path.GetExtension(files[files.Count - 1]) == ".mpxproj")
                    {
        ...
        {
            WriteLine("Usage:");
            WriteLine("ModPhuserEx.CLI -n|noPause <project configuration>");
            WriteLine("ModPhuserEx.CLI -n|noPause -o|out=<output directory> <modules>");
        ...
        private class ConsoleLogger : ILogger
        {
            ...
            public void Finish(bool successful)
            {
                ...
                if (successful)
                {
                    Console.Title = "ModPhuserEx - Success";
                    WriteLineWithColor(ConsoleColor.Green, "Finished " + timeString);
                    ReturnValue = 0;
                }
                else
                {
                    Console.Title = "ModPhuserEx - Fail";
